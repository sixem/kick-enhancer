...
;(() => {
  ...
  if (!requestBuild) {
    reportUnavailable(
      'reinstall the development loader to grant request access',
    )
    return
  }
  ...
  function fetchBuild(onSuccess) {
    ...
    try {
      const result = requestBuild(details)
      // Modern managers may reject a promise instead of invoking onerror.
      Promise.resolve(result).catch(details.onerror)
    } catch {
      details.onerror()
    }
  }
