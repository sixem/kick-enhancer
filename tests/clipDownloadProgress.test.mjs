...
test('waits for a useful sample before estimating transfer rate', () => {
  assert.equal(
    estimateDownloadTransfer(1024, 2048, 1_000, 1_999),
    undefined,
  )
  assert.equal(
    estimateDownloadTransfer(0, 2048, 1_000, 3_000),
    undefined,
  )
})
...
test('omits remaining time when size is unknown or complete', () => {
  assert.deepEqual(
    estimateDownloadTransfer(4096, undefined, 0, 2_000),
    {
      bytesPerSecond: 2048,
  ...
  assert.deepEqual(
    estimateDownloadTransfer(4096, 4096, 0, 2_000),
    {
      bytesPerSecond: 2048,
      remainingSeconds: undefined,
    },
